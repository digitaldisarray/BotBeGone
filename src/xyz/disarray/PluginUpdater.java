...
public class PluginUpdater {
	...
	public boolean needsUpdate() {
		...
		try {
			...
			if (newVersionAvailiable(this.plugin.getDescription().getVersion(),
					this.version.replaceAll("[a-zA-z ]", ""))) {
				if (this.out) {
					this.plugin.getLogger().log(Level.INFO, " New Version found: {0}",
							this.version.replaceAll("[a-zA-z ]", ""));
	...
	public void update() {
		try {
			...
			try {
				if (this.out) {
					this.plugin.getLogger().log(Level.INFO, "Trying to download from: {0}{1}",
							new Object[] { getFolder(this.pluginurl), this.downloadURL });
				}
